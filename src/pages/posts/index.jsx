...
export const Posts = () => {
    ...
    const onSearch = () => {
        ...
        xHttp.open("GET", `http://localhost:3000/post?titulo=${searchText}`, true)
    ...
    return <div className={container}>
        ...
        {posts.map(post => <div className={postContainer}>
            ...
            {post.arquivos.filter(arquivo => arquivo.mimetype.includes('jpeg') || arquivo.mimetype.includes('png')).map(imagem =>
                <img src={`http://localhost:3000/image/${imagem.filename}`} className={image} />
            )}
            {post.arquivos.filter(arquivo => !arquivo.mimetype.includes('jpeg') && !arquivo.mimetype.includes('png')).map(file =>
                <div className={fileContainer}><a href={`http://localhost:3000/file/${file.filename}`}>{file.originalname}</a></div>
            )}
        </div>)}
    </div>
}
